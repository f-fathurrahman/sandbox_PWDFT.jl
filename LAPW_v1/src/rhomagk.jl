...
function rhomagk!(
    ...
    # muffin-tins contributions to density and magnetization
    #
    # loop over all atoms
    for ia in 1:Natoms
        isp = atm2species[ia]
        ...
        for j in 1:nstsv
            ...
            if abs(wo) < epsocc
                #println("State = $j is skipped because wo = $wo")
                continue
            end
            #
            wo = wo*wppt
            ...
            if spinpol
                ...
                if ncmag
                    ...
                    error("Noncollinear magnetism is not supported yet")
                else
                    # collinear
                ...
                #@printf("ia,ist = %4d %4d sum rhomt[ia] = %18.10f\n", ia, j, sum(rhomt[ia]))
            end 
