...
function parse_namelist(filename::AbstractString)
    ...
    while i <= n
        ...
        if startswith(line, "&")
            ...
            println("group_lines = ", group_lines)
...
function parse_group(lines::Vector{String})
    ...
    # Remove any stray '&' tokens (group delimiters)
    filter!(t -> t != "&", tokens)
    ...
    for (idx, eq_idx) in enumerate(eq_positions)
        ...
        lhs_tokens = String[]
        j = eq_idx - 1
        # Stop when we hit a previous '=' or ',' (assignment delimiter)
        while j >= 1 && tokens[j] != "=" && tokens[j] != "," &&
              (is_variable_name(tokens[j]) || tokens[j] == "(" ||
