...
# Normalize a matrix A to be unitary. If X is a matrix with orthogonal columns and A a non-singular matrix, then L�wdin-orthogonalizing X*A is equivalent to computing X*normalize_matrix(A)
