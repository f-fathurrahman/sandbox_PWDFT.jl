...
function read_special_kpts( filebands::String )
    ...
    for ik = 1:Nkpt_spec
        ...
        elseif (symb_kpts_spec[ik] == "S")
            symb_kpts_spec[ik] = L"$\Sigma$"
        elseif (symb_kpts_spec[ik] == "S1")
            symb_kpts_spec[ik] = L"$\Sigma_{1}$" 
        end
    end
    close(f)
...
function main_plot_bands()
    ...
    E_f = 0.4061198043 # maximum(ebands[:,10])
    println("E_f = ", E_f)
    # From SCF data
    # Fermi energy =       0.4061198043 Ha =      11.0510827651 eV
    ...
    fig = @pgf Axis( {
             ...
             xmin=kmin,
             xmax=kmax,
             ymin=-8.0, # Set the minimum y-axis limit
             ymax=2.0,  # Set the maximum y-axis limit
          },
          plt_inc...)
