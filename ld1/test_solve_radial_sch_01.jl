...
function init_pot(zeta, r, Nrmesh)
    ...
    for i in 0:Nrmesh
        vpot[i+1] = - zeta / r[i+1]
        #vpot[i+1] = - 2 * zeta / r[i+1]
    end
    return vpot
end

function solve_sheq(n, l, Nrmesh, dx, r, sqr, r2, vpot, zeta)
    maxiter = 100
    eps = 1.0e-10
    ...
    eup = vpot[end]
    #elw = minimum(sqlhf ./ r2 .+ vpot)
    elw = minimum( sqlhf ./ (2.0 .* r2) .+ vpot )
    if eup - elw < eps
        println(elw, " ", eup)
        error("ERROR: solve_sheq: lower and upper bounds are equal")
    end
    ...
    for kkk in 1:maxiter
        ...
        for i in 1:Nrmesh
            ...
            #f[i+1] = ddx12 * (sqlhf + r2[i+1]*(vpot[i+1] - e))
            if f[i+1] == 0.0
                f[i+1] = 1.0e-20
            end
    ...
    if !converged
        ...
        error("error in solve_sheq: too many iterations")
    else
...
function main()
    ...
    e, y = solve_sheq(n, l, Nrmesh, dx, r, sqr, r2, vpot, zeta)

    @printf("Energies in Ry\n")
