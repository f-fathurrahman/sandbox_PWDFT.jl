...
function main()
    ...
    psp_gth = PsPot_GTH(joinpath(DIR_PSP, "C-q4.gth"))

    psp_gthnum = PsPot_UPF("/home/efefer/pseudo/HGH/C.pz-hgh.UPF")
    interp_table = build_interp_proj(psp_gthnum, ecutwfc, CellVolume, cell_factor)

    ibeta = 1
    Gm = 0.0
    Vq = eval_proj_interp(interp_table, ibeta, Gm)
    println("Vq (numeric)  = ", Vq)

    Vq = eval_proj_G(psp_gth, 0, 1, Gm, CellVolume)
    println("Vq (analytic) = ", Vq)
end
